...
namespace JonsEngine
{
        ...
        mReadbackLatency(RenderSettingsToVal(readbackLatency)), mCurrFrame(0), mContext(context), mSDSMInitialShader(nullptr), mSDSMFinalShader(nullptr), mSDSMCBuffer(device, context, mSDSMCBuffer.CONSTANT_BUFFER_SLOT_COMPUTE)
    {
        ...
        while (width > 1 || height > 1)
        {
            ...
            mDepthReductionRTVs.emplace_back( device, context, DXGI_FORMAT_R16G16_UNORM, width, height, true, false );
        }
    ...
    void DX11DepthReductionPass::ReduceDepth( float& minDepth, float& maxDepth, const RenderSettings& renderSettings )
    {
        ...
        mContext->CSSetUnorderedAccessViews(UAV_SLOT_0, 1, &initialRTV.mUAV.p, nullptr);
        ...
        for (uint32_t index = 1; index < mDepthReductionRTVs.size(); ++index)
        {
            ...
            mContext->CSSetUnorderedAccessViews(UAV_SLOT_0, 1, &rtv.mUAV.p, nullptr);
