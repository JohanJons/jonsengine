...
namespace JonsEngine
{
    ...
    ModelNode::ModelNode(const PackageNode& pkgNode, const InitDataList& initData, NodeContainer& nodes, MeshContainer& meshes) :
        mName(pkgNode.mName), mLocalAABB(pkgNode.mAABB.mMinBounds, pkgNode.mAABB.mMaxBounds), mLocalTransform(pkgNode.mTransform), mImmediateChildNodes(ParseImmediateChildNodes(pkgNode, initData, nodes, meshes)),
        mAllChildNodes(ParseAllChildNodes(pkgNode, initData, nodes, meshes)), mMeshes(ParseMeshes(meshes, initData, pkgNode))
    {
    }
    ...
    ModelNode::ModelNode(NodeContainer& nodes, MeshContainer& meshes, const std::string& name, const Vec3& minBounds, const Vec3& maxBounds, const Mat4& initialTransform, const DX11MeshID meshID) :
        mName(name), mLocalAABB(minBounds, maxBounds), mLocalTransform(initialTransform), mImmediateChildNodes(ImmediateChildrenIter(nodes.end()), ImmediateChildrenIter(nodes.end())), mAllChildNodes(nodes, GetStartIndex(nodes), GetStartIndex(nodes)), mMeshes(ParseMesh(meshes, name, minBounds, maxBounds, meshID))
    {
    }
