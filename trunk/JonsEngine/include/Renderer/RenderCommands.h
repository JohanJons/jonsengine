...
namespace JonsEngine
{
    ...
    {
        RenderableLighting(const Mat4& viewMatrix, const Vec4& gamma, const Vec3& viewDirection, const uint32_t usedLights) : mViewMatrix(viewMatrix), mGamma(gamma), mViewDirection(viewDirection), mUsedLights(usedLights)
        {
        }
        ...
        struct Light
        {
            Light() : mLightColor(1.0f, 1.0f, 1.0f, 0.0f), mLightPosition(0.0f), mLightDirection(0.0f), mLightType(0), mIntensity(1.0f), mRadius(0.0f), mMaxDistance(0.0f)
            {
            }
