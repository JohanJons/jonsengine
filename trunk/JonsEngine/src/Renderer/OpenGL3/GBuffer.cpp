...
{
    GBuffer::GBuffer(Logger& logger, const uint32_t shadingProgramID, const uint32_t windowWidth, const uint32_t windowHeight) : mLogger(logger), mFramebuffer(0), mDepthTexture(0), mTextureSampler(0)
    {
        ...
        // depth
        GLCALL(glGenTextures(1, &mDepthTexture));
        GLCALL(glBindTexture(GL_TEXTURE_2D, mDepthTexture));
        GLCALL(glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, windowWidth, windowHeight, 0, GL_DEPTH_COMPONENT, GL_FLOAT, 0));
        GLCALL(glFramebufferTexture2D(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_TEXTURE_2D, mDepthTexture, 0));
...
/*        GLCALL(glUniform1i(glGetUniformLocation(shadingProgramID, "unifPositionTexture"), GBuffer::GBUFFER_TEXTURE_POSITION));
    ...
    GBuffer::~GBuffer()
    {
        ...
        GLCALL(glDeleteTextures(1, &mDepthTexture));
