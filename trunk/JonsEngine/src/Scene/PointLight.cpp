...
{
    PointLight::PointLight(const std::string& name, SceneNodePtr node) : mName(name), mSceneNode(node), mHashedID(boost::hash_value(name)), mLightColor(1.0f), mFalloffFactor(1.0f), mMaxDistance(0.0f)
    {
    }
